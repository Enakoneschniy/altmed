	...
	$.fn.exists = function() {
		...
		scroll: function() {
			if(this.active) {
				...
				clearTimeout(this.t), 70 > e && this.$header.removeClass("-white"), i && (0 == this.opacity && "up" == t ? (this.show = e, a > e ? this.show = 0 : this.show -= 70) : 1 == this.opacity && "down" == t && (this.show = e), this.show = Math.max(0, this.show)), this.$header.hasClass("-open") && (this.show = e), this.hover && (this.show = e);
				var o = e - this.show;
				o = Math.max(0, o), o = Math.min(o, 70);
				var n = (70 - o) / 70;
				this.$header.css("opacity", n), e > a ? this.$header.addClass("-white") : 0 == n && this.$header.removeClass("-white"), this.y = e, this.direction = t, this.opacity = n
			}
		}
		...
		open: function() {
			var e = this.$a,
				t = this.$img;
			this.active = !0, this.y = $(window).scrollTop(), e.addClass("-calculate");
			var i = t.width(),
				s = t.height(),
				a = s / i * 100;
			...
			e.append('<div class="placeholder" style="padding-bottom:' + a + '%"></div>'), e.addClass("-active"), t.css({
				top: 0,
				left: 0,
				width: e.width(),
				height: e.height()
			}), setTimeout(function() {
				e.addClass("-zoom"), t.css({
					top: r,
					left: h,
					width: i,
					height: s
		...
		close: function() {
			var e = this.$a,
				t = this.$img;
			this.active = !1, e.removeClass("-zoom"), t.css({
				top: 0,
				left: 0,
				width: e.width(),
				height: e.height()
			}), setTimeout(function() {
				e.removeClass("-active"), e.children(".placeholder").remove(), t.css({
					top: "auto",
					left: "auto",
					width: "auto",
					height: "auto"
...
function($) {
	js.site = js.model.extend({
		...
		ready: function() {
			$("#on-this-page").each(function() {
				...
				$("#content h2").each(function() {
					...
					$(this).attr("id", i), e.append('<li><a href="#' + i + '">' + t + "</a></li>")
				})
			}), $("#hero").each(function() {
				var e = 500;
		...
		ready: function() {
			...
			$(".twitter-count").each(function() {
				...
				i.attr({
					...
					return window.open($(this).attr("href"), "newwindow", "width=500,height=400"), !1
		...
		ready: function() {
			...
			e.exists() && (this.$form = e, this.$field = $(".field", e), this.$input = $('input[type="text"]', e), this.$results = $('<div class="field-results"></div>'), this.$field.append(this.$results))
		},
		submit: function(e) {
			e.preventDefault()
		},
		clear: function(e) {
			e.preventDefault(), this.$form.find('input[type="text"]').val("").trigger("keyup")
		},
		keyup: function(e) {
			var t = this,
				i = this.$input.val();
			if(i != this.search) {
				if(this.search = i, this.$form.removeClass("is-open is-loading"), !i || i.length < 3) return this.$results.html(""), void this.abort();
				this.abort(), this.$form.addClass("is-loading"), this.timeout = setTimeout(function() {
					t.fetch()
				}, 250)
			}
		},
		abort: function() {
			this.xhr && this.xhr.abort(), clearTimeout(this.timeout)
		},
		fetch: function() {
			var e = this;
			this.xhr = $.ajax({
				...
				success: function(t) {
					e.$form.addClass("is-open"), e.$results.html(t), e.sort_results()
				},
				complete: function() {
		...
		init: function(e) {
			if(!e.hasClass("ninput")) {
				...
				"checkbox" != s && "hidden" != s && "submit" != s && (a || (a = i.text(), t.attr("placeholder", a)), i.exists() || (i = $("<label></label>"), t.before(i)), i.text(a), e.addClass("ninput"), this.render(e))
			}
		},
		render: function(e) {
			var t = e.find("[name]"),
				i = e.find("label");
			e.removeClass("-f -v"), t.is(":focus") && e.addClass("-f"), t.val() && e.addClass("-v")
		},
		_render: function(e) {
			this.render(e.$el)
		}
